...
<html xml:lang="<?= $language;?>" lang="<?= $language;?>">
  <head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
      ...
      Asset::getInstance()->addCss(SITE_TEMPLATE_PATH . "/css/lib/air-datepicker.css");
      Asset::getInstance()->addCss(SITE_TEMPLATE_PATH . "/css/guests.css");
      ...
      if($APPLICATION->GetCurPage()=='/partners/') {
          Asset::getInstance()->addCss(SITE_TEMPLATE_PATH . "/css/partners.css");
      }
      if(str_contains($APPLICATION->GetCurPage(),'/rooms/')) {
      Asset::getInstance()->addCss(SITE_TEMPLATE_PATH ."/css/rooms.css");
      }
      if(str_contains($APPLICATION->GetCurPage(),'/calendar/')) {?>


          <link rel="stylesheet" href="<?=SITE_TEMPLATE_PATH?>/css/lib/fancybox.css">
          <link rel="stylesheet" href="<?=SITE_TEMPLATE_PATH?>/css/lib/swiper.min.css">

      <?}
      if($APPLICATION->GetCurPage()=='/contacts/') {
      Asset::getInstance()->addCss(SITE_TEMPLATE_PATH ."/css/contacts.css");
      }
      if($APPLICATION->GetCurPage()=='/privacy/') {
          Asset::getInstance()->addCss(SITE_TEMPLATE_PATH ."/css/privacy.css");
      }
      if($APPLICATION->GetCurPage()=='/about/event-otel/') {
      Asset::getInstance()->addCss(SITE_TEMPLATE_PATH ."/css/about.css");
      }
      if($APPLICATION->GetCurPage()=='/404.php') {
          Asset::getInstance()->addCss(SITE_TEMPLATE_PATH ."/css/notFound.css");
      }
      if($APPLICATION->GetCurPage()=='/about/glemping/') {
          Asset::getInstance()->addCss(SITE_TEMPLATE_PATH ."/css/good.css");
      }
      if($APPLICATION->GetCurPage()=='/organization/') {
          Asset::getInstance()->addCss(SITE_TEMPLATE_PATH ."/css/organization.css");
      }
      if($APPLICATION->GetCurPage()=='/houseRules/') {
          Asset::getInstance()->addCss(SITE_TEMPLATE_PATH ."/css/houseRules.css");
      }
      if($APPLICATION->GetCurPage()=='/services/') {
          Asset::getInstance()->addCss(SITE_TEMPLATE_PATH ."/css/services.css");
      }
      if($APPLICATION->GetCurPage()=='/gallery/') {
          Asset::getInstance()->addCss(SITE_TEMPLATE_PATH ."/css/lib/fancybox.css");
    ...
    <link rel="preload" href="<?=SITE_TEMPLATE_PATH?>/fonts/Involve-Regular.woff2" as="font" type="font/woff2" crossorigin/>
    <link rel="preload" href="<?=SITE_TEMPLATE_PATH?>/fonts/LibelSuit-Regular.woff2" as="font" type="font/woff2" crossorigin/>
    <link rel="preload" href="<?=SITE_TEMPLATE_PATH?>/fonts/Onest-Regular.woff2" as="font" type="font/woff2" crossorigin/>

  </head>
  <?$APPLICATION->ShowPanel();?>
  <body>
  <?
  if($APPLICATION->GetCurPage()=='/' or $APPLICATION->GetCurPage()=='/about/glemping/')  {
  ...
  <div class="container">
    <div class="header__inner">
      ...
      >
        <img width="150" height="40" class="header__logo-big" src="<?=SITE_TEMPLATE_PATH?>/img/header/logo-big.svg" alt="логотип Конаково"/>

        <img width="35" height="35" class="header__logo-small" src="<?=SITE_TEMPLATE_PATH?>/img/header/logo-small.svg" alt="логотип Конаково"/>
      </a>

      <address class="header__address">
        <span class="header__address-icon">
          <img src="<?=SITE_TEMPLATE_PATH?>/img/icons/geo.svg" alt="иконка локации" />
        </span>
      ...
      <div class="menu">
        ...
        <div class="menu__content">
          ...
          <div class="menu__details">
            ...
            <ul class="menu__details-list">
              <li class="menu__details-link">
                <a
                  href="tel:+74959746544"
                  aria-label="позвонить по номеру : +7 (495) 974-65-44"
          ...
          <!-- <a href="/reservation/?date=<?=date("Y-m-d")?>&nights=1&adults=2&children=1&children-age=&utm_source=google" class="link link--green">забронировать</a> -->
        </div>
      </div>
      ...
      <?
      $main='';
     if(stripos($APPLICATION->GetCurPage(), '/calendar/') !== false){
    $main = 'main-calendar';
}?>
      <main class="main cust <?=$main?>">
          <?php
          // Проверяем, что это главная страница
          if ($APPLICATION->GetCurPage() == '/') {
          // Проверяем, не показывали ли мы прелоадер ранее (через куки)
          if (!isset($_COOKIE['preloader_shown'])) {
          // Устанавливаем куку на 1 день
          setcookie('preloader_shown', 'Y', time() + 86400, '/');
          ?>
          <div class="preloader">
